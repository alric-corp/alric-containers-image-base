...
| CE2E-07 | Publicação por digest para o(s) repositório(s) ECR corporativo(s) autorizado(s) | `skopeo copy --all --preserve-digests`, sem `CreateRepository`/`PutImageTagMutability` no perfil recorrente (Profile A) | Containers Products (executa) / Cloud/IAM (aprova destino) | Sim | NOT RUN |
